...
const CoachPage = ({ history, match }) => {
  const [quantity, setQuantity] = useState(1);
  const [rating, setRating] = useState(0);
  const [review, setReview] = useState("");
  const [allReviews, setAllReviews] = useState([]);
  const [hasOrderedItem, setHasOrderedItem] = useState(false); // bool to check if the user has ordered this coach
  const [coachReviewForm, setCoachReviewForm] = useState(false); // bool to decide whether to coach the review form or not
  ...
  const coachCreateReview = useSelector((state) => state.coachCreateReview);
  const {
  ...
  // add a new review, and reset the stored coach review in the redux store
  useEffect(() => {
  ...
  useEffect(() => {
    if (coach && coach.reviews && userInfo) {
      let flag = 0; // to check if this user has already reviewed this coach
      for (let review of coach.reviews) {
        if (review.user === userInfo.id) {
          flag = 1;
          break;
        }
      }
      flag ? setCoachReviewForm(false) : setCoachReviewForm(true);
    } else {
      setCoachReviewForm(true);
    }
  }, [coach, userInfo]);
    ...
    if (orders && orders.length) {
      ...
      for (let obj of orders) {
        for (let ele of obj.orderItems) {
          if (ele.coach.toString() === match.params.id) {
            flag = 1;
            break;
          }
        }
  ...
  useEffect(() => {
    if (coach && coach.reviews) {
      const sortedArr = coach.reviews.sort(
  ...
  return (
    <>
      ...
      {coach && (!coach._id || coach._id !== match.params.id) ? (
        <Loader />
      ) : error ? (
              ...
              <Col className="overflow-auto">
                <div className="scrollbar" id="style-3">
                  <div>
                    ...
                    <div>
                      <strong>*Lưu ý:</strong> <br /> {coach.description3}{" "}
                      <br />
                      <br />
                    </div>
              ...
              <Col className="col-4">
                ...
                <Col>
                  <ListGroup variant="flush">
                    <ListGroup.Item>
                      <h7 className="fs-5">{coach.name}</h7>
                    </ListGroup.Item>
                          ...
                          <Row>
                            <Col>
                              ...
                              >
                                ...
                                {[...Array(coach.countInStock).keys()].map(
                                  (ele) => (
